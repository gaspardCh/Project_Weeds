...
image = Image.open(os.path.join(folder_path, images_list[0]))
...
def click_prediction():
    ...
    my_text = "label : " + images_list[index] + " | prediction : " + class_prediction + " | probability : " + str(
        np.around(prediction[0][index_max_prediction] * 100, decimals=2)) + " %."
...
bouton = CTk.CTkButton(fenetre, text="Predict random image", command=click_prediction)
bouton.pack()
